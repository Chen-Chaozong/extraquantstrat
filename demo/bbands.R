library(extraquantstrat)
suppressWarnings(rm("order_book.bbands",pos=.strategy))
suppressWarnings(rm("account.bbands","portfolio.bbands",pos=.blotter))
suppressWarnings(rm("account.st","portfolio.st","stock.str","startDate","initEq",'start_t','end_t'))
...
# lets add some rules
add.stoplosssig(strategy = strat.st,sigcol = "Cl.lt.LowerBand",type = "stopPercent",
                arguments = list(
                  column="Close",
                  percent=0.03,
                  type="dynamic",
...
add.rule(strategy = strat.st,name='ruleSignal',
         arguments = list(sigcol="Cl.lt.LowerBand",
                          ...
                          osFUN=osMaxPos),
         type='enter')
add.stoplossrule(strategy = strat.st,name='ruleSignal',
                 arguments = list(sigcol="lt.stop",
                                  sigval=TRUE,
                                  orderqty= 100, 
                                  ordertype='market',
                                  orderside="short",
...
start_t<-Sys.time()
out<-try(apply_Strategy(strategy='bbands' , portfolios='bbands',parameters=list(sd=SD,n=N)) )
...
#
# $Id: bbands.R 1724 2015-12-14 12:54:08Z bodanker $
#
###############################################################################
